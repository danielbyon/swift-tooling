...
Options:
  ...
  --release-version VERSION    Release version for --release-archive
...
if [[ -z "$release_archive" ]]; then
    ...
    release_version=${release_version:-$(parse_manifest_value RELEASE_VERSION "$manifest_file")}
...
LOCK
    ...
    if [[ ! -f "$adapter_target" || "$current_lock_version" != "$release_version" ]]; then
...
commit_staged_files() {
    ...
    local adapter_was_present=0
    local lock_was_present=0
    local local_config_was_present=0
    local gitignore_was_present=0
    local release_changed=0
    local adapter_changed=0
    local lock_changed=0
    local local_config_changed=0
    local gitignore_changed=0

    restore_all() {
        if [[ "$gitignore_changed" -eq 1 ]]; then
            rm -f "$gitignore_target"
            if [[ "$gitignore_was_present" -eq 1 ]]; then
                mv "$gitignore_backup" "$gitignore_target"
            fi
        fi
        if [[ "$local_config_changed" -eq 1 ]]; then
            rm -f "$local_config_target"
            if [[ "$local_config_was_present" -eq 1 ]]; then
                mv "$local_config_backup" "$local_config_target"
            fi
        fi
        if [[ "$lock_changed" -eq 1 ]]; then
            rm -f "$lock_target"
            if [[ "$lock_was_present" -eq 1 ]]; then
                mv "$lock_backup" "$lock_target"
            fi
        fi
        if [[ "$adapter_changed" -eq 1 ]]; then
            rm -f "$adapter_target"
            if [[ "$adapter_was_present" -eq 1 ]]; then
                mv "$adapter_backup" "$adapter_target"
            fi
        fi
        if [[ "$release_changed" -eq 1 ]]; then
            rm -rf "$install_directory"
            if [[ "$release_was_present" -eq 1 ]]; then
